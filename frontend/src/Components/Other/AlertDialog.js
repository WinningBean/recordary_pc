...
class ConnectWebSocket extends React.Component {
  ...
  componentDidMount () { // 웹소켓에 필요한 script 추가면 (사이트 접속 혹은 사용자 로그인시 구동하면 됨 - 모든 페이지에 있어야 함) * 채팅부분 함께 사용
    ...
    sockjs.src = "https://cdnjs.cloudflare.com/ajax/libs/sockjs-client/1.3.0/sockjs.min.js";
    sockjs.async = true;
  ...
  sendNotice(noticeType, activeCd, targetCd){ // 이벤트가 발생하면 메세지를 보내준다
    ...
    // 사용자가 그룹초대를 수락할 경우 : sendNotice("GROUP_MEMBER_NEW", 멤버가 된 사용자 코드, 그룹 코드);
    // 사용자가 그룹을 탈퇴할 경우 : sendNotice("GROUP_MEMBER_AWAY", 좋아요한 사용자 코드, 좋아요된 게시물);
    ...
    // 사용자가 일정멤버 초대를 수락할 경우 : sendNotice("SCHEDULE_MEMBER_ALLOW", 멤버가 된 사용자 코드, 스케줄 코드);
    ...
    if (this.isStomp)
      this.socket.send('/pub/notice', {}, JSON.stringify(data)); // 이벤트 발생 메세지를 '/ws-stomp/pub/notice'(this.socket의 endpoint + 매핑주소) 주소로 보내준다. * 같은 방식으로 채팅부분에 사용
    else
  ...
  userConnectStomp(){ // 사용자의 알람 토픽 구독 (사용자 로그인 성공시 연결해줘야 함)
    ...
    client.connect({}, function () { // * 같은 방식으로 채팅부분에 사용
      console.log("Connect Stomp");

      client.subscribe('/topic/user' + 로그인유저코드, function (response) { // 로그인 사용자의 해당 토픽을 구독한다
  ...
  checkNoticeType(data){ // NoticeType 체크 후 사용자입장에서 알람 디자인 구성
    ...
    if (data.noticeType == "GROUP_APPLY_COME") ;
    if (data.noticeType == "GROUP_APPLY_INVITE") ;
    if (data.noticeType == "GROUP_APPLY_COME_NOT") ;
    if (data.noticeType == "GROUP_APPLY_INVITE_NOT") ;
    if (data.noticeType == "GROUP_MEMBER_ALLOW") ;
    if (data.noticeType == "GROUP_MEMBER_NEW") ;
    if (data.noticeType == "GROUP_MEMBER_AWAY") ;
    if (data.noticeType == "POST_LIKE_NEW") ;
    if (data.noticeType == "POST_GROUP_NEW") ;
    if (data.noticeType == "SCHEDULE_MEMBER_INVITE") ;
    if (data.noticeType == "SCHEDULE_MEMBER_INVITE_NOT") ;
    if (data.noticeType == "SCHEDULE_MEMBER_ALLOW") ;
    if (data.noticeType == "COMMENT_NEW") ;
    if (data.noticeType == "COMMENT_SUB_NEW") ;
  }
  ...
  notice_GROUP_MEMBER_ALLOW(activeCd){
    const activeGroup = (await axios.get(`/group/${activeCd}`)).data;
    this.setState({noticeMsg : "<a href='/" + activeUser.userId + "'>" + activeGroup.groupNm + "이 회원의 그룹신청을 수락하였습니다.</a>"});
  }
  ...
  notice_GROUP_MEMBER_AWAY(){
    const activeUser = (await axios.get(`/user/${activeCd}`)).data;
    const targetGroup = (await axios.get(`/group/${targetCd}`)).data;
    this.setState({noticeMsg : "<a href='/" + activeUser.userId + "'>" + activeUser.userId + "(" + activeUser.userNm + ")님이 " + targetGroup.groupNm + "를 탈퇴하였습니다.</a>"});
  }

  notice_POST_LIKE_NEW(){
    const activeUser = (await axios.get(`/user/${activeCd}`)).data;
    const targetPost = (await axios.get(`/post/${targetCd}`)).data;
    this.setState({noticeMsg : "<button onclick='게시물 팝업창 띄우기'>" + activeUser.userId + "(" + activeUser.userNm + ")님의 회원님의 " + targetPost.postCd + "번 게시물을 좋아합니다.</button>"});
  }

  notice_POST_GROUP_NEW(activeCd, targetCd){
    const activeGroup = (await axios.get(`/group/${activeCd}`)).data;
    const targetPost = (await axios.get(`/post/${targetCd}`)).data;
    this.setState({noticeMsg : "<button onclick='게시물 팝업창 띄우기'>" + targetGroup.groupNm + "의 게시물이 등록되었습니다.</button>"});
  }
  ...
  notice_COMMENT_SUB_NEW(activeCd, targetCd){
    const activeComment = (await axios.get(`/comment/${activeCd}`)).data;
    const popupPost = (await axios.get(`/post/${activeComment.postCd}`)).data;
